...
class DownloadPage(webapp.RequestHandler):
	def get(self):
		...
		if ((target < 0 or 2 < target) or (range_begin > range_end) or
				(target == 0 and (range_begin != 0 or range_end != 0)) or
				(target == 1 and (range_begin >= journal_num or range_end > journal_num)) or
				(target == 2 and (range_begin >= resource_num or range_end > resource_num))):
			self.error(405)
			self.response.headers['Content-Type'] = 'text/plain; charset="UTF-8"'
